...
const recordsSeq = useSeqLoad(); const statsSeq = useSeqLoad();
...
async function loadStudents() { const classId = currentClassId.value; if (!classId) return; const next = await api.students.list({ class_id: classId, status: '在读' }); if (classId === currentClassId.value) students.value = next; } async function loadGroups() { const classId = currentClassId.value; if (!classId) return; const next = await api.duties.list({ class_id: classId }); if (classId !== currentClassId.value) return; duties.value = next; if (!dutyGroups.value.some(group => group.no === selectedDutyGroup.value)) selectedDutyGroup.value = dutyGroups.value[0]?.no || null; } async function loadRecords() { const classId = currentClassId.value; if (!classId) return; const mySeq = recordsSeq.seq(); const next = await api.assessment.records.list({ class_id: classId, month: recordMonth.value || undefined, include_voided: includeVoided.value ? '1' : undefined }); if (!recordsSeq.isStale(mySeq) && classId === currentClassId.value) records.value = next; }
async function loadOverview() { const classId = currentClassId.value; if (!classId) return; const next = await api.assessment.records.list({ class_id: classId, month: localMonth() }); if (classId === currentClassId.value) overviewRecords.value = next; }
async function loadStats() { const classId = currentClassId.value; if (!classId) return; const mySeq = statsSeq.seq(); const next = statsPeriod.value === 'monthly' ? await api.assessment.stats.monthly({ class_id: classId, month: statsMonth.value }) : await api.assessment.stats.term({ class_id: classId, academic_year: currentClass.value?.academic_year, term: currentClass.value?.term }); if (!statsSeq.isStale(mySeq) && classId === currentClassId.value) stats.value = next; }
