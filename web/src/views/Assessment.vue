    ...
    <el-tabs v-model="activeTab" class="assessment-tabs">
        ...
        <div class="assessment-grid">
          ...
          <el-card shadow="never" class="stream-card">
            ...
            <el-table :data="records" size="small" border max-height="540">
              ...
              <el-table-column label="操作" width="170" fixed="right"><template #default="{ row }"><el-button link type="primary" @click="openEdit(row)">编辑</el-button><el-button link @click="showRevisions(row)">历史</el-button><el-button v-if="row.status === 'active'" link type="danger" @click="voidRecord(row)">撤销记录</el-button><el-button v-else link type="success" @click="restoreRecord(row)">恢复记录</el-button></template></el-table-column>
            </el-table>
      ...
      <el-tab-pane label="规则管理" name="rules">
        ...
        <el-card v-for="category in categories" :key="category.id" shadow="never" class="rule-card">
          <template #header><div class="card-title"><b>{{ category.name }}</b><span class="muted">{{ category.isActive ? '启用中' : '已停用' }}</span><el-button text @click="openCategoryDialog(category)">编辑分类</el-button></div></template>
          <el-table :data="category.items" size="small"><el-table-column prop="name" label="行为项目" /><el-table-column prop="description" label="说明" /><el-table-column label="固定分值" width="110"><template #default="{ row }"><span :class="row.score < 0 ? 'negative' : 'positive'">{{ row.score > 0 ? '+' : '' }}{{ row.score }}</span></template></el-table-column><el-table-column label="重复规则" width="160"><template #default="{ row }">{{ row.allowDailyRepeat ? '允许当天重复' : '每天一次' }}</template></el-table-column><el-table-column label="操作" width="170"><template #default="{ row }"><el-button link type="primary" @click="openItemDialog(row, category.id)">编辑</el-button><el-button v-if="row.isActive" link type="danger" @click="disableItem(row)">停用</el-button></template></el-table-column></el-table>
        </el-card>
      </el-tab-pane>
    </el-tabs>
...
const selectedCategoryId = ref(null); const targetMode = ref('manual'); const recordMonth = ref(''); const includeVoided = ref(false); const statsPeriod = ref('monthly'); const statsMonth = ref(localMonth());
...
const groupTargets = computed(() => targetMode.value === 'all' ? students.value.map(student => student.id) : targetMode.value === 'leaders' ? [...new Set(duties.value.filter(row => row.role !== '值日生').map(row => row.student_id))] : targetMode.value === 'duty' ? [...new Set(duties.value.filter(row => row.role === '值日生').map(row => row.student_id))] : scoreForm.value.studentIds);
...
async function loadStudents() { students.value = await api.students.list({ class_id: currentClassId.value, status: '在读' }); } async function loadGroups() { duties.value = await api.duties.list({ class_id: currentClassId.value }); } async function loadRecords() { if (!currentClassId.value) return; records.value = await api.assessment.records.list({ class_id: currentClassId.value, month: recordMonth.value || undefined, include_voided: includeVoided.value ? '1' : undefined }); }
...
async function disableItem(item) { try { await ElMessageBox.confirm(`停用「${item.name}」？历史记录会保留。`, '确认停用', { type: 'warning' }); await api.assessment.items.disable(item.id); await loadCategories(); ElMessage.success('行为项目已停用'); } catch { /* 取消 */ } }
...
watch(selectedCategoryId, () => { if (!selectedItems.value.some(item => item.id === scoreForm.value.itemId)) scoreForm.value.itemId = selectedItems.value[0]?.id || null; }); watch(() => store.currentClassId, reloadAll); onMounted(reloadAll);
</script>
