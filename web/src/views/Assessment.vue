<template>
  <section class="page-shell assessment-page" v-loading="loading">
    ...
    <el-tabs v-model="activeTab" class="assessment-tabs">
      <el-tab-pane label="日常记分" name="records">
        <div class="assessment-grid">
          ...
          <el-card shadow="never" class="stream-card">
            ...
            <el-table :data="records" size="small" border max-height="540">
              <el-table-column prop="behaviorDate" label="日期" width="105" /><el-table-column prop="student_name" label="学生" width="90" /><el-table-column prop="itemName" label="行为" min-width="130" />
              <el-table-column label="分值" width="70" align="center"><template #default="{ row }"><span :class="row.score_snapshot < 0 ? 'negative' : 'positive'">{{ row.score_snapshot > 0 ? '+' : '' }}{{ row.score_snapshot }}</span></template></el-table-column>
              <el-table-column label="状态" width="80"><template #default="{ row }"><el-tag v-if="row.status === 'voided'" type="info" size="small">已撤销</el-tag><span v-else>有效</span></template></el-table-column>
              <el-table-column label="操作" width="250" fixed="right"><template #default="{ row }"><div class="assessment-actions"><el-button class="row-btn row-btn-edit" :icon="EditPen" @click="openEdit(row)">编辑</el-button><el-button class="row-btn" :icon="Clock" @click="showRevisions(row)">历史</el-button><el-button v-if="row.status === 'active'" class="row-btn row-btn-del" :icon="CircleClose" @click="voidRecord(row)">撤销</el-button><el-button v-else class="row-btn row-btn-restore" :icon="CircleCheck" @click="restoreRecord(row)">恢复</el-button></div></template></el-table-column>
            </el-table>
      ...
      <el-tab-pane label="统计视图" name="stats">
        ...
        <div class="stats-chart-grid">
          <el-card shadow="never" class="visual-card"><template #header><div class="card-title"><b><el-icon><TrendCharts /></el-icon> 学生净分走势</b><span class="muted">前 10 名</span></div></template><EChart :option="rankingChartOption" height="280px" /></el-card>
          <el-card shadow="never" class="visual-card"><template #header><div class="card-title"><b>分类贡献分布</b><span class="muted">净分</span></div></template><EChart :option="categoryChartOption" height="280px" /></el-card>
        </div>
    ...
    <el-dialog v-model="historyVisible" title="查看修正历史" width="680px"><el-timeline><el-timeline-item v-for="revision in revisions" :key="revision.id" :timestamp="revision.createdAt" placement="top"><b>{{ revision.action === 'edit' ? '编辑' : revision.action === 'void' ? '撤销' : '恢复' }}</b><p>修正原因：{{ revision.reason || '未填写' }}</p><pre>{{ JSON.stringify({ before: revision.before, after: revision.after }, null, 2) }}</pre></el-timeline-item></el-timeline><el-empty v-if="!revisions.length" description="暂无修正历史" /></el-dialog>
  </section>
</template>
...
import { Refresh, EditPen, Clock, CircleClose, CircleCheck, CollectionTag, TrendCharts } from '@element-plus/icons-vue';
...
const rankingChartOption = computed(() => ({ tooltip: { trigger: 'axis' }, grid: { left: 42, right: 18, top: 18, bottom: 42 }, xAxis: { type: 'category', data: (stats.value.ranking || []).slice(0, 10).map(row => row.name), axisLabel: { interval: 0, rotate: 25, color: '#685f55' } }, yAxis: { type: 'value', minInterval: 1 }, series: [{ type: 'bar', data: (stats.value.ranking || []).slice(0, 10).map(row => ({ value: row.net, itemStyle: { color: row.net >= 0 ? '#8bd6af' : '#f35b3f', borderRadius: [5, 5, 0, 0] } })) }] }));
const categoryChartOption = computed(() => ({ tooltip: { trigger: 'item' }, legend: { bottom: 0, type: 'scroll', textStyle: { color: '#685f55' } }, series: [{ type: 'pie', radius: ['38%', '68%'], center: ['50%', '44%'], label: { formatter: '{b}\n{c}分' }, data: (stats.value.categories || []).filter(row => row.net !== 0).map(row => ({ name: row.categoryName, value: Math.abs(row.net) })) }] }));
...
.assessment-actions, .rule-actions { display:flex; flex-wrap:wrap; align-items:center; gap:6px; }
.assessment-actions .row-btn, .rule-actions .row-btn { margin:0 !important; padding:4px 9px; min-height:28px; border:2px solid var(--ink); border-radius:999px; box-shadow:2px 2px 0 var(--ink); background:#fffdf4; font-size:12px; }
...
@media (max-width:640px) { .target-scope-note { align-items:flex-start; flex-wrap:wrap; } .duty-group-select { width:100%; margin-left:0; } .assessment-actions, .rule-actions { gap:4px; } }
</style>
