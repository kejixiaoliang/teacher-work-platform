...
const categoryChartOption = computed(() => ({ tooltip: { trigger: 'item' }, legend: { bottom: 0, type: 'scroll', textStyle: { color: '#685f55' } }, series: [{ type: 'pie', radius: ['38%', '68%'], center: ['50%', '42%'], label: { formatter: '{b}\n{c}分' }, data: categoryRows.value.map(row => ({ name: row.categoryName, value: Math.abs(row.net) })) }] }));
