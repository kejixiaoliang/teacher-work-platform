...
Page({
  ...
  openProfile(event) {
    ...
    const collection = type === 'metric' ? 'student_metrics_history' : 'student_records';
    const records = type === 'metric' ? this.data.metrics : this.data.records;
    const existing = uuid ? records.find((item) => item.uuid === uuid) : null;
    this.setData({ profileType: type, profileEditing: true, profileForm: existing ? { ...existing } : type === 'metric' ? { height_cm: '', vision_left: '', vision_right: '', is_myopia: false, source: '手动测量', date: new Date().toISOString().slice(0, 10) } : { type: '表现', content: '', date: new Date().toISOString().slice(0, 10), remark: '' } });
  },
  ...
  async saveProfile() { const type = this.data.profileType; const form = this.data.profileForm; const collection = type === 'metric' ? 'student_metrics_history' : 'student_records'; if (type === 'record' && !String(form.content || '').trim()) return wx.showToast({ title: '请填写成长记录内容', icon: 'none' }); this.setData({ saving: true }); const action = form.uuid ? 'update' : 'create'; const result = await callStudentProfile({ action, collection, datasetId: this.data.datasetId, classUuid: this.data.classUuid, studentUuid: this.data.uuid, uuid: form.uuid, record: form }); this.setData({ saving: false }); if (!result?.ok) return wx.showToast({ title: result?.errors?.[0] || '保存档案记录失败', icon: 'none' }); this.cancelProfile(); await this.loadStudent(); wx.showToast({ title: '档案记录已保存', icon: 'success' }); },
