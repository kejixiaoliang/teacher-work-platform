...
Page({
  data: { datasetId: '', classUuid: '', classIndex: 0, classes: [], allStudents: [], students: [], records: [], roleIndex: 0, studentIndex: 0, remark: '', editingUuid: '', loading: false, saving: false, error: '', roles: ROLES },
  ...
  selectClass(event) { const classIndex = Number(event.detail.value); const cls = this.data.classes[classIndex]; if (!cls) return; this.setData({ classIndex, classUuid: cls.uuid, students: this.data.allStudents.filter((item) => item.classUuid === cls.uuid && item.status !== '离校'), records: [], editingUuid: '', roleIndex: 0, studentIndex: 0, remark: '' }); this.loadRecords(); },
  async loadRecords() { const classUuid = this.data.classUuid; const result = await callBusinessData({ action: 'query', collection: 'duties', datasetId: this.data.datasetId, classUuid }); if (classUuid !== this.data.classUuid) return; const names = new Map(this.data.students.map((item) => [item.uuid, item.name || '未命名学生'])); const records = (result?.records || []).filter((item) => ROLES.includes(item.role)).map((item) => ({ ...item, studentName: names.get(item.studentUuid) || '未命名学生' })); this.setData({ records, error: result?.ok ? '' : (result?.errors?.[0] || '读取课代表数据失败') }); },
  ...
  async save() { const student = this.data.students[this.data.studentIndex]; const role = ROLES[this.data.roleIndex]; const editingUuid = this.data.editingUuid; if (!student) return wx.showToast({ title: '请选择学生', icon: 'none' }); const conflict = this.data.records.find((item) => item.uuid !== editingUuid && item.role === role); if (conflict) return wx.showToast({ title: '科目冲突：同一科目只能设置一名课代表', icon: 'none' }); this.setData({ saving: true }); const result = await callBusinessData({ action: editingUuid ? 'update' : 'create', collection: 'duties', datasetId: this.data.datasetId, classUuid: this.data.classUuid, uuid: editingUuid || undefined, record: { role, studentUuid: student.uuid, gender: student.gender || '', remark: this.data.remark } }); this.setData({ saving: false }); if (!result?.ok) return wx.showToast({ title: result?.errors?.[0] || '保存失败', icon: 'none' }); this.cancelEdit(); wx.showToast({ title: '已保存', icon: 'success' }); this.loadRecords(); },
