...
Page({
  ...
  async loadRecords() { const result = await callBusinessData({ action: 'query', collection: 'duties', datasetId: this.data.datasetId, classUuid: this.data.classUuid }); this.setData({ records: (result?.records || []).filter((item) => item.role && !item.role.endsWith('课代表')), error: result?.ok ? '' : (result?.errors?.[0] || '读取班委数据失败') }); },
