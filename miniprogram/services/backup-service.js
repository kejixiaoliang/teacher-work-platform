...
export async function exportBackupFile({ datasetId, classUuid = '', className = '教师工作台' } = {}) {
  ...
  const response = await callCloudFunction('backup-data', { action: 'export', datasetId, ...(classUuid ? { classUuid } : {}) });
