...
对于不带托盘、窗口关闭即退出的版本，还必须验证 sidecar 生命周期：窗口关闭事件进入 `RunEvent::WindowEvent` 后用 `destroy()` 绕过 WebView `beforeunload` 阻塞，再请求应用退出；窗口 `Destroyed` 也要请求退出；在 Tauri 的退出事件中统一取出并终止 `Child`，随后调用 `wait()` 回收，不能在事件回调中直接调用 `std::process::exit(0)`。否则可能出现窗口已经关闭但主程序或内置 Node 仍残留，进而阻塞更新、卸载和数据库文件释放。
