const EXCHANGE_FORMAT = 'teacher-work-backup';
...
const ASSESSMENT_COLLECTIONS = ['categories', 'items', 'records', 'revisions'];
...
function validateExchangeEnvelope(payload) {
  const errors = [];
  ...
  if (!isObject(payload.attachments)) {
    ...
    if (!Number.isSafeInteger(payload.attachments.omittedCount) || payload.attachments.omittedCount < 0) errors.push('attachments.omittedCount 无效');
  }
...
module.exports = { validateExchangeEnvelope };
