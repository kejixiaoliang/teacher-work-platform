...
function fixture({ packageVersion = '0.2.0', tauriVersion = '0.2.0', cargoVersion = '0.2.0', changelogVersion = '0.2.0' } = {}) {
...
test('matching product versions satisfy the build contract', () => {
  assert.deepEqual(validateVersionContract(fixture()), { version: '0.2.0' });
});

test('Cargo product version mismatch identifies Cargo.toml', () => {
  assert.throws(() => validateVersionContract(fixture({ cargoVersion: '0.1.0' })), /Cargo\.toml.*0\.2\.0/);
});

test('Tauri product version mismatch identifies tauri.conf.json', () => {
  assert.throws(() => validateVersionContract(fixture({ tauriVersion: '0.1.0' })), /tauri\.conf\.json.*0\.2\.0/);
});

test('missing current release notes identifies Changelog.vue', () => {
  assert.throws(() => validateVersionContract(fixture({ changelogVersion: '2026.08' })), /Changelog\.vue.*0\.2\.0/);
});
